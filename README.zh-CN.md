...
Claude Code 等 AI 编程 CLI 功能强大，但它们运行在终端里。这意味着没有持久化的面板、没有可视化的 Diff 审查、没有跨会话历史、也无法自由切换多个 API 平台。CC GUI 用原生桌面 UI 包装这些 CLI，补上终端无法提供的那些能力层 —— 同时保持所有数据**本地存储**。（调用远程模型 API 需要联网；应用本身没有云端后台。）
...
| **CLI 会话导入** | 发现并导入已有的 Claude Code CLI 会话到 CC GUI |
...
从 [Releases](https://github.com/hejq-reolink/cc-gui/releases) 下载最新 `.dmg` —— 通用二进制，同时支持 Apple Silicon 和 Intel Mac。
...
欢迎贡献！请通过 [Issue](https://github.com/hejq-reolink/cc-gui/issues) 提交 Bug 报告或功能建议，也欢迎提交 Pull Request。
